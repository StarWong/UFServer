...
const
  ...
  Tables_Ex: array [0 .. 9] of string = ('9', 'UA_Account_Ex', 'UA_Period_Ex',
...
type
  ...
  private
   {$REGION 'Conn&Query'}
    ...
    FSaPsw: string;
    FBakDir: string;
    procedure _Log(str:RawByteString);
  ...
  {$REGION 'Public'}
    ...
    procedure PlanToCleanDirs(const Root:string;Days:Word = 31);
...
procedure Tmybackuptask._Log(str:RawByteString);
var
   hFile:  THandle;
   sFileName: string;
   //Str: RawByteString;
...
try
Result:=False;
DirName:=FormatDateTime('YYYYMMDD',Now);
...
procedure Tmybackuptask.PlanToCleanDirs(const Root:string;Days:Word =31);
var
